...
public class JetpackParticleHandler {
    ...
    public static void spawnExhaustParticles(ParticleSpawnType particleSpawnType, Player player) {
        ...
        if (player.isInWater()) {
            ...
            spawnBoostNozzle(mc, player, fuelDef, base.subtract(right), look);
            spawnBoostNozzle(mc, player, fuelDef, base.add(right), look);
        } else {
            if (hovering) {
                base = base.subtract(front.scale(0.25));
            }
            spawnNozzle(mc, player, fuelDef, base.subtract(right));
            spawnNozzle(mc, player, fuelDef, base.add(right));
        }
    }

    private static void spawnNozzle(Minecraft mc, Player player, FuelTypeDefinition fuelDef, Vec3 pos) {
        if (mc.level == null) return;
    ...
    private static void spawnBoostNozzle(Minecraft mc, Player player, FuelTypeDefinition fuelDef, Vec3 pos, Vec3 look) {
        ...
        mc.level.addParticle(fuelDef.boostParticle(), pos.x, pos.y, pos.z, vx, vy, vz);
    }
}
