...
public class JetpackFuelItemHandler implements IItemHandler {
    ...
    public @NotNull ItemStack getStackInSlot(int slot) {
        ...
        return new ItemStack(fuel.type().getItem(), fuel.count());
    }

    @Override
    public @NotNull ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
        if (stack.isEmpty()) return ItemStack.EMPTY;
        var type = FuelType.fromItem(stack.getItem());
        if (type.isEmpty()) return stack;

        FuelData current = be.getFuelData();
        int currentCount = current != null ? current.count() : 0;
        if (current != null && current.type() != type.get()) return stack;

        int canAdd = FuelData.MAX_COUNT - currentCount;
        if (canAdd <= 0) return stack;

        int toAdd = Math.min(stack.getCount(), canAdd);
        if (!simulate) be.setFuelData(new FuelData(type.get(), currentCount + toAdd));
    ...
    public boolean isItemValid(int slot, ItemStack stack) {
        return FuelType.fromItem(stack.getItem()).isPresent();
    }
}
